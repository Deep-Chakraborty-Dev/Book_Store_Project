...
const CheckoutPage = () => {
    ...
    const [createAnOrder, { isLoading }] = useCreateOrderMutation()
    ...
    const onSubmit = async (data) => {
        const newOrder = {
            ...
            phone: data.phone,
            // supply the field expected by the schema
            productIds: cartItems.map(item => item._id),
            // older clients may still inspect a `products` array; include it
            // for convenience (not stored by schema unless added).
            products: cartItems.map(item => ({
                productId: item._id,
                title: item.title,
                quantity: 1, // change if you allow multiple qty
            })),
                            ...
                            <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-3 my-8">
                                ...
                                <div className="lg:col-span-2">
                                    <div className="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
                                        ...
                                        <div className="md:col-span-5 flex justify-center items-center mt-4">
                                        <img 
                                            src={googlePay} 
                                            alt="Google Pay" 
