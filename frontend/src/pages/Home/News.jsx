        ...
        
        {
            news.map((item,index) => (
                <SwiperSlide key={index}>
                    <div className='flex flex-col sm:flex-row sm:justify-between items-center gap-12 sm:text-sm'>
                        <div className='py-4'>
                            <Link to={"/"}>
                                <h3 className='text-lg font-medium hover:text-emerald-500 mb-4'>
                                    {item.title}
                                </h3>
                            </Link>
                            <div className='w-10 h-1 bg-amber-300 mb-5'></div>
                            <p className='text-gray-400 text-sm'>{item.description}</p>
                        </div>
                        <div className='shrink-0'>
                            <img src={item.image} alt='' className='w-4/5 sm:w-1/2 object-cover transition-transform duration-200 hover:scale-110'></img>
                        </div>
                    </div>
                </SwiperSlide>
            ))
